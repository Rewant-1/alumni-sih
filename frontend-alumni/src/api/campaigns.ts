...
const MOCK_CAMPAIGNS: Campaign[] = [
  {
    ...
    shortDescription: 'Support bright students who need financial assistance to continue their education.',
    description: `The Merit Scholarship Fund aims to provide financial support to deserving students who demonstrate exceptional academic performance but face financial constraints.

## Why This Matters
Every year, talented students are forced to drop out due to financial difficulties. Your contribution can help change lives and create future leaders.

## How Funds Will Be Used
- Tuition fee support
- Book and study material assistance
- Hostel accommodation
- Monthly stipends for living expenses

## Impact So Far
- 50+ students supported last year
- 95% graduation rate among scholarship recipients
- Many alumni have become donors themselves`,
  ...
  },
  {
    ...
    description: `We are raising funds to establish a modern computer laboratory equipped with the latest technology for the Computer Science department.

## Features
- 100 high-performance workstations
- Latest software licenses
- VR/AR equipment for research
- 24/7 access for students`,
  ...
  },
  {
    ...
    description: `Support groundbreaking artificial intelligence research conducted by our faculty and students.
...
- Healthcare AI`,
  ...
  },
  {
    ...
    description: `Help our student athletes compete at national and international levels by providing them with world-class equipment and facilities.`,
    category: 'Sports',
    goalAmount: 500000,
    raisedAmount: 350000,
    donorsCount: 156,
...
export const getAllCampaigns = async (params?: CampaignSearchParams): Promise<PaginatedResponse<Campaign>> => {
  try {
    const response = await apiClient.get<ApiResponse<Campaign[] | PaginatedResponse<Campaign>>>('/campaigns', { params });
    const backendData = response.data.data;
    // Handle both formats: direct array or paginated response with items
    if (Array.isArray(backendData)) {
      // Backend returns { data: [...], pagination: {...} }
      const pagination = (response.data as { pagination?: { total?: number; page?: number; limit?: number; pages?: number } }).pagination;
      return {
        items: backendData,
